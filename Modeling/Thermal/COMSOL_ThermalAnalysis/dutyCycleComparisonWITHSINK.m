...
extractedValues = zeros(length(dutyCycles),4); %time constant, asym temp, max temp, failure time
...
for i = 1:length(dutyCycles)
    ...
    filename = strcat(folderName,'\dutyCycle', num2str(dutyCycle), '.csv');
    ...
    extractedValues(i,:) = [timeConstant (asymTemp-273.15) (maxTemp-273.15) failureTime];
...
hold off;
figure;
scatter(dutyCycles, extractedValues(:,1), 'DisplayName', '\tau');
title('\tau (s) vs pulse spacing (s)');
ylabel("\tau, s")
xlabel("duty cycle")

figure;
hold on;
scatter(dutyCycles, extractedValues(:,2), 'DisplayName', 'asymptote temp');
scatter(dutyCycles, extractedValues(:,3), 'DisplayName', 'max temp over 45s');
ylabel("temp, C")
xlabel("duty cycle")
...
hold off;
figure;
scatter(dutyCycles, extractedValues(:,4), 'DisplayName', 'failure time')
